...
3. Discover every build setting exposed by the active Xcode whose name begins with `SWIFT_UPCOMING_FEATURE_` and require each one to be `YES` at project level. Treat the settings listed in the shared guide as the Xcode 27 minimum, not as a future-exhaustive list.
4. Enumerate every target and configuration, including unit-test and UI-test targets, and inspect effective values with Xcode project-aware tooling or `xcodebuild -showBuildSettings`. Remove redundant target copies and treat a disabling or different target override as a violation unless an exact exception applies.
